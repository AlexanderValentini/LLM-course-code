...
# Taken from the official Llama implementation:
...
class Attention(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        self.compute_query = nn.Linear(config.dim, config.n_heads * self.head_dim, bias=False) #Compute query matrix from input embeddings (across heads). One vector per embedding per head.
        self.compute_key = nn.Linear(config.dim, self.n_kv_heads * self.head_dim, bias=False) #Compute key matrix from input embeddings (across heads).
        self.compute_value = nn.Linear(config.dim, self.n_kv_heads * self.head_dim, bias=False) #Compute value matrix from input embeddings (across heads).
        self.compute_output = nn.Linear(config.n_heads * self.head_dim, config.dim, bias=False) #Output projection matrix to project the attention output back to the original embedding dimension.
    ...
    def compute_query_key_value_scores(self,
        ...
        attn_scores = torch.matmul(query, key.transpose(-1, -2)) / math.sqrt(self.head_dim) # (m, n_heads, seq_len_q, head_dim) @ (m, n_heads, head_dim, seq_len) -> (m, n_heads, seq_len_q, seq_len)
        
        mask = torch.tril(torch.ones(seqlen_q, seqlen_k, device=query.device)).bool()
        attn_scores = attn_scores.masked_fill(mask == 0, float('-inf'))
                
        attn_scores_weights = F.softmax(attn_scores, dim=-1) # (m, n_heads, seq_len_q, seq_len)
        attn_scores_weights = self.attn_dropout(attn_scores_weights) # apply dropout to the attention weights. # (m, n_heads, seq_len_q, seq_len)

        output = torch.matmul(attn_scores_weights, value) # (m, n_heads, seq_len_q, seq_len) @ (m, n_heads, seq_len, head_dim) -> (m, n_heads, seq_len_q, head_dim)
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        # todo
        norm_x = self.attention_norm(x) # apply layer normalization to the input
        h = x + self.attention(norm_x) # apply self-attention to the normalized input. With residual connection. 
        norm_h = self.ffn_norm(h) # apply layer normalization to the self-attention output
        output = h + self.feed_forward.forward(norm_h) # apply feed-forward network to the normalized self-attention output. With residual connection.
        return output
#        raise NotImplementedError
...
class Llama(LlamaPreTrainedModel):
    ...
    def forward(self, tokens: torch.Tensor, targets: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if targets is not None:
            ...
            logits = self.output(h) #(batch_size, seq_len, config.vocab_size)
        else:
            # inference-time mini-optimization: only forward the output on the very last position
            logits = self.output(h[:, [-1], :]) # note: using list [-1] to preserve the time dim. (batch_size, 1, config.vocab_size)
...
#            raise NotImplementedError
            ...
            if temperature == 0.0:
                ...
                idx_next = torch.argmax(logits, dim=-1, keepdim=True) # The keepdim=True argument controls the shape of the output tensor.  when you perform a reduction operation
                #like argmax, sum, or mean along a dimension, that dimension is normally "squeezed" or removed from the output shape. keepdim=True keeps the number of dimensions of
                #the tensor the same, with the reduced dimension having a size of 1.
                #Dimensions of "logits" are (batch_size, 1, config.vocab_size)
            else:
                '''
                ...
                #Temperature is also descri in slide 16+17 in the Modern NLP lecture on page nr 31.
                logits = logits / temperature # scale the logits by the temperature
                probabilities = F.softmax(logits, dim=-1) # normalize the logits to probabilities
